...
**Versions are tagged, and the tag is what preserves them.** `v1.0` is the June site at `730c59a`, tagged immediately before the overwrite, because once `main` moved nothing pointed at those commits and git eventually discards what nothing points to. `v2.0` is the launch at `52603dd`. `v2.1` is `b49c5e0` (11 September 2026): the site name and favicons, below. Tag each future launch; from now on the history is linear so nothing else is at risk.

**To ship one change without the rest of `develop`, go through a hotfix branch from `main`**, first used for `v2.1`. Create it in a git worktree in the scratchpad, verify it against a build of `main` (which should match the live site), fast-forward `main` to it, then cherry-pick the same commit onto `develop` so the two do not drift. **The worktree needs `git -c core.longpaths=true`**: the scratchpad path is long enough that learning-world content paths pass Windows' 260-character limit, and without the flag the checkout fails partway. It failed that way once, and because the command was chained with `;`, everything after it ran in the `develop` tree instead. Chain with `&&`, and check the worktree's file count before building from it. A `main` build differs from live only in the `?v=` asset hashes, because the Windows checkout has CRLF endings and GitHub's build does not.

**The site name Google shows is "Amit G Dusane", and three signals must keep agreeing**: the `WebSite` structured data on the home page (`schema.html`, with `alternateName` "amitdusane.com"), `og:site_name`, and the home page title, now "Adobe Experience Cloud, Explained | Amit G Dusane". Until `v2.1` the title ended "| amitdusane.com", and with the signals disagreeing Google showed the bare domain. **Favicons come in 48, 96 and 192px, rendered from `favicon.svg`**, because Google recommends at least 48px for search results, and `favicon.ico` sits at the root, which used to return the 404 page. Google re-reads both on its own schedule; Search Console's URL Inspection on the home page is the only way to prompt it.

**GA decides from the baseURL, not from the branch** (4 September 2026). `layouts/partials/head.html` fires the tracking call only when `.Site.BaseURL` contains `amitdusane.com`, which is the same test the noindex guard uses in `layouts/index.headers` and a few lines further down in `head.html` itself. A production build passes no `-b`, so the baseURL in `hugo.toml` stands and the site tracks; a staging build passes `-b $CF_PAGES_URL/` and a `hugo server` build serves from localhost, so neither one does. **The branches no longer differ in any way, and a launch is an ordinary push with nothing to remember.** Until that day the call sat inside a comment wrapper that had to be deleted before shipping to `main` and put back on `develop` the same hour, and forgetting the second edit sent tester traffic into the live property during the week the numbers matter most.

**The two guards are one decision and must stay that way.** Anything that turns tracking on also turns noindex off, so they can never disagree about which build is the real site. The one case that would break the symmetry is a staging copy hosted under amitdusane.com itself rather than on pages.dev: both guards would invert together, and both would then need something narrower than `strings.Contains`. Nothing is hosted that way today.

**The Cloudflare staging copy was deleted on 4 September 2026, straight after launch, and it is back.** It ran from 2 September for tester feedback, and was deleted rather than paused because testers had the URL: `noindex` keeps a copy out of Google but does nothing about a person forwarding a link, and a public second copy that drifts out of date the moment work resumes is worse than none. `site-architecture.md` records every setting needed to stand it back up, including the two that are load-bearing and not obvious.

**As of 7 September 2026 `amitdusane-stage.pages.dev` is live again and correctly configured**, verified against the running site rather than the documentation: it serves `X-Robots-Tag: noindex, nofollow, noarchive` on every path, a `noindex, nofollow` meta on all 241 HTML pages, `Disallow: /` in robots.txt, and no analytics of any kind. Its canonicals point at the deployment's own pages.dev address, which proves the build is still passing `-b $CF_PAGES_URL/`. Production is the exact mirror: tracking on, no robots meta, no `X-Robots-Tag`, `Allow: /`. **Whoever rebuilds it again must keep that flag**, and the count above is the check.

**A third protection was added on 7 September 2026, and it answers a question the other two cannot.** Amit reported that visits to the staging copy appeared to be reaching GA4. They were not, and the guards were doing their job — but investigating it exposed a real gap. The build-time test answers *what baseURL was this built with*, which is not the same question as *where is this page being served from*. Three ordinary situations separate them, and in every one the build-time test alone says yes when the honest answer is no: a staging project rebuilt without `-b`, so it inherits the `hugo.toml` baseURL; a production build copied or mirrored anywhere else; and a local `hugo server` run with `-b https://amitdusane.com/` to check canonicals.

So `head.html` now replaces Hugo's `_internal/google_analytics.html` with its own snippet that **refuses to load gtag unless `location.hostname` is exactly `amitdusane.com` or `www.amitdusane.com`.** The property ID still comes from `hugo.toml`, so there is still one place to change it. This does not break the two-guards-are-one-decision rule above: it is a second, narrower gate on the same decision, and it can only ever withhold tracking, never enable it somewhere the build-time test already refused. The noindex guard needs no equivalent, because a meta tag is decided at build time and cannot be reconsidered in the browser; its run-time protection is the `X-Robots-Tag`.

**It also closes the one case this file previously recorded as unhandled.** A staging copy hosted at `staging.amitdusane.com` passes `strings.Contains` and would have tracked; the exact-match hostname test blocks it. Verified end to end: a build carrying the production baseURL, served over HTTP from `localhost`, emits the snippet, loads no gtag, makes no request to googletagmanager and leaves `window.gtag` undefined.

**The report that started this was real, and the cause was the 404 page.** Amit's GA4 Pages report for 6–7 September showed views of `/aep-mobile-sdk/mobile-basics/debug-and-release/` and four of its neighbours, on days those pages existed only on `develop`. That looks exactly like staging leaking into the live property, and the first round of checking wrongly concluded there was nothing to find, because it only asked whether staging tracked. It does not. Every one of those rows was **a 404 on amitdusane.com**: the page genuinely does not exist there, Hugo serves `404.html` through this same `head.html`, and gtag defaults `page_location` to the URL that was requested. So a missing page was recorded under its own path and was indistinguishable in the Pages report from a page that exists.

**Two lessons, and the second is the general one.** A row in a Pages report is not evidence a page exists; it is evidence somebody asked for that path. And when a measurement looks impossible, check what the live site actually returns for that exact URL before concluding the collection is at fault — the answer here was one 404 lookup away, and reasoning about the guards produced a confident wrong answer instead.

`head.html` now sends 404s as `page_location` `/404` with the attempted path in `page_title` as `404: /the/path`. They are still tracked, because a list of paths people ask for and do not get is worth having, but they no longer masquerade as content. Only `404.html` gets the override; every real page keeps the plain config, which is asserted by a build check of exactly one page carrying `page_location`.

**Anything already collected stays.** Rows from before this change keep the requested path, and traffic collected between 2 and 4 September 2026, when tracking was still switched by a hand-edited comment wrapper, predates all of it. A GA4-side data filter on hostname is the only thing that stops unwanted data being *accepted* rather than merely not *sent*, and that is configuration in GA4 rather than anything in this repository. Note also that visiting amitdusane.com to verify something is itself a tracked hit, including from a session doing the checking.
...
**A "world" is what remains after that**: a top-level `.Section` with its own stylesheet, JS and content shell. Five exist: `/web-sdk-migration/` (blue accent, `world-shell.css`), `/aep-mobile-sdk/` (teal, the same stylesheet), `/adobe-analytics-learning/` (crimson, `world-learning.css`), and the two Basics sections on `world-basics.css`, `/mobile-app-basics/` (terracotta) and `/website-basics/` (plum). The switch is a lookup in `[params.worlds]` in `hugo.toml`, keyed on `.Section`, carrying each world's `css`, `js`, `root` and `shape`; `shape` is `curriculum`, `procedure` or `basics`, and it decides which shell `baseof.html` renders. **Adding a world is a config block, not a new branch** — INF-05 and INF-06, done 20 Aug 2026, deliberately before CJA becomes the third one. Each world also owns its own `phases`; they used to sit in a single global `[params.phases]` that any world using `wherefits.html` or `homemap.html` would silently inherit.
...
**The everyday form of that trap is the clock, not the config. It fired on 30 Aug 2026, and again on 6 Sep 2026 writing the first Mobile SDK step.** This machine runs IST, five and a half hours ahead of UTC, so **the danger window runs from 18:30 IST right through to 05:30 IST the next morning**, not merely until midnight. An earlier version of this note said "until midnight" and that is what let the second one through: the page was dated from the local calendar at 03:47 IST, when UTC was still the previous day, so `published` was in the future and Hugo dropped the page without a word. The build reported success and the count simply failed to rise.

**Date front matter from `date -u`, never from the local calendar or from what the session says today is.** Run it, read it, use that. The page-count assertion is what catches this, which is the entire reason that check exists, and on both occasions it is the only thing that did.
...
**Assert the page count after every build.** Current baseline: **273 pages on a production build, 274 on a staging build** (11 September 2026). It rose from 257 when Website Basics added its front page, search index, feed, glossary and twelve topics, from 254 when Mobile App Basics became a section of its own, and from 249 before that, when the Mobile SDK spine went from fourteen steps to fifteen and step six gained four platform pages of its own.
...
- **No inline citation markers, anywhere.** Amit, 11 September 2026: a `[23]` after a sentence is a distraction, and it signals low confidence in our own content, the same failure as deferring to Adobe in the prose. A procedure guide lists its sources on its References page and nowhere else; the learning world's `ref-box` is the handoff there. All 91 markers were removed from both guides the same day.
...
**A procedure guide no longer carries its own basics, and its rail does not link to them.** The Mobile SDK guide's Part One became Mobile App Basics on 11 September 2026. Where a step needs that vocabulary, the step says so in its prose and links the Basics section, and its Why? row links the exact topic. That opens in the same side panel as a KB article: the topic's one-sentence summary and the opening of its "Why it matters to you", read from the topic page itself, then a link to the full topic. A "Start here" rail row was built and removed the same day at Amit's request.

**The side panel must always be closable** (11 September 2026). It starts below the fixed site header, which is drawn above everything and once hid the panel's close button; on a phone it is 88% wide, so a strip of page stays visible; and a tap anywhere outside it closes it.

**And it shows prose, never a dump.** The panel reads the target page itself and takes only its opening paragraphs, stopping at the first table, code block, figure or heading, at three paragraphs, or at about 110 words. It used to show the first three "sentences" of the search index's plain text, and plain text does not know what a table is: on the variable mapping catalog, which is mostly a table, every cell ran together into one wall of words. Amit caught it on migration step 2. All 49 Why? targets across both guides were audited against the new reader the same day, with none empty, none over 110 words and none mostly code.
...
Mobile App Basics at `/mobile-app-basics/` is the first instance, fourteen topics moved out of the Mobile SDK guide with every old URL aliased. **Website Basics at `/website-basics/` is the second**, twelve topics written on 11 September 2026 as the test of the container. It needed a registry block, an accent pair, a home-page tile colour, a one-line list template and one container fix: the line under every headline, which said "apps" for every Basics section, is now the registry's `remind`. **The name is "Website Basics", never "Web Basics".**

**Nothing in a Basics section is numbered**, because there is no order to promise. The reader arrives knowing some of the field and none of the rest, so the front page shows every topic at once, in groups. **Keep that page simple** (Amit, 11 September 2026): the title, one lead line, then every topic as a plainly tappable row with an arrow, because a phone has no hover to say "this opens". **The premise, "What this section is", comes first**, between the lead and the list, because it is what a learner has to read before anything else. Putting it under the list, the order the learning landings use, was tried and reversed the same day. A word picker that lit up topics by unfamiliar word was built and removed the same day: fourteen topics do not need a tool with a learning curve, and on a phone it filled the first screen. **Orienting is done by "See where this fits"** on every topic, the learning world's pocket map with two tiers, groups then topics (`wherefits.html`, `wherefits.js` shared by both worlds). From 1240px it heads the topic's side column; below that it is the floating green tab.
...
**From 1240px a topic takes the learning world's three-column measurements** (Amit, 11 September 2026): a 280px rail, a 760px reading column centred in the space that remains, and a 250px right rail against the page edge, 40px from the text (`--b-rail`, `--measure`, `--b-side`, `--b-side-gap` in `world-basics.css`). The first version centred a 1120px block, which left empty page to the right of a side column that still had to scroll. The right rail uses the learning rail's small type, and each term runs on one line with its meaning, the way a printed glossary sets them. With that, and **no topic carrying more than six words**, it fits without scrolling on every topic at every size measured, down to 1280x720. iOS and Android carried eight and still scrolled on a 1366x768 screen, so Amit had both trimmed to six the same day. Each cut was a term the topic's own body never uses or already explains; a term a topic's key question depends on stays, which is why Scheme survived on iOS.
