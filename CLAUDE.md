...
**If pages.dev traffic ever does appear in GA4, the place to look is the Hostname dimension against a date range**, since anything collected between 2 and 4 September 2026 predates all of this and stays in the property permanently. A GA4-side data filter on hostname is the only thing that stops unwanted data being *accepted* rather than merely not *sent*, and that is configuration in GA4 rather than anything in this repository.
